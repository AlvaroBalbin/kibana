...
Zoom access token
:   A Zoom Server-to-Server OAuth access token. Generate one through the Zoom Marketplace or API (refer to [Get credentials](#zoom-credentials)). Tokens expire after 1 hour.
...
## Get credentials [zoom-credentials]
