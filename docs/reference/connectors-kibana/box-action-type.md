...
Authentication
:   Connects through Box's OAuth 2.0 Authorization Code flow. Requires a Client ID and Client Secret from the Box MCP server integration in the Admin Console. Refer to [Get credentials](#box-credentials) for setup instructions.
...
## Get credentials [box-credentials]
