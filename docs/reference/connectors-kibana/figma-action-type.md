...
API key (header)
:   A Figma personal access token sent in the `X-Figma-Token` header. See [Get credentials](#figma-credentials).

OAuth 2.0 authorization code
:   Uses an **OAuth app** registered in Figma. In {{kib}} you provide:

    - **Client ID** and **Client Secret**: from your Figma OAuth app
    - **Redirect URI**: register {{kib}}'s OAuth callback in your Figma OAuth app (see [Get credentials](#figma-credentials))
...
## Get credentials [figma-credentials]
