...
**Access token (bearer)**
:   Authenticates using a static token. Use a **Service Key** (recommended) from HubSpot Development, or a **Private App** access token (starts with `pat-`). See [Get credentials](#hubspot-credentials) for both options.
...
## Get credentials [hubspot-credentials]
